...
public:
                                                   ...
                                                   double min_area, double max_area)
  {
    ...
    for (const auto &contour : contours) {
      ...
      constexpr double kMaxFrameAspectRatio = 4.0;
      if (short_side <= 1.0 || diagnostic.aspect_ratio > kMaxFrameAspectRatio) {
                                         ...
                                         const std::vector<cv::Point> &outer) const
  {
    ...
    for (std::size_t i = 0; i < contours.size(); ++i) {
      ...
      if (fitted && cv::isContourConvex(refined)) {
        quad = std::move(refined);
      } else {
        // When line-fitting fails (e.g. a large nut indentation leaves
        // too few support points on one side), the hull quad is a usable
        // fallback. Its corners may be 1–2 px off but it preserves the
        // straight inner edges that a nut cannot bend.
        quad.clear();
        cv::approxPolyDP(hull, quad, 0.02 * cv::arcLength(hull, true), true);
        if (quad.size() != 4 || !cv::isContourConvex(quad)) continue;
      }
                              ...
                              const std::vector<cv::Point> &fallback_inner)
  {
    ...
    // Searches a binary mask for the best basket candidate using the same
    // geometry checks. Used for the primary and fallback mask passes.
    auto find_basket_in_mask = [&](const cv::Mat &mask, const std::string &) -> bool {
      std::vector<std::vector<cv::Point>> contours;
      cv::findContours(mask, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
      double best = image_area * config_.min_basket_area_ratio;
      bool found = false;
      for (const auto &contour : contours) {
        double area = cv::contourArea(contour);
        cv::Moments m = cv::moments(contour);
        if (area < best || std::abs(m.m00) < 1e-6) continue;
        ...
        const auto rrect = cv::minAreaRect(contour);
        const double rectangle_area = rrect.size.area();
        ...
        // Store the minAreaRect angle for release orientation alignment.
        geometry.basket_angle_deg = rrect.angle;
        if (rrect.size.width < rrect.size.height) geometry.basket_angle_deg += 90;
        while (geometry.basket_angle_deg < -45) geometry.basket_angle_deg += 180;
        while (geometry.basket_angle_deg > 135) geometry.basket_angle_deg -= 180;
        found = true;
      }
      return found;
    };
    bool basket_found = find_basket_in_mask(blue_mask, "blue_hsv");
    if (!basket_found) {
      // The configured narrow blue range can miss the basket under varying
      // light. Broaden S and V thresholds; tight H still prevents false
      // positives from non-blue table/floor regions.
      cv::Mat fallback_blue;
      cv::inRange(hsv,
        cv::Scalar(config_.blue_h_min, std::max(20, config_.blue_s_min - 40),
                   std::max(30, config_.blue_v_min - 40)),
        cv::Scalar(config_.blue_h_max, 255, 255), fallback_blue);
      cv::morphologyEx(fallback_blue, fallback_blue, cv::MORPH_CLOSE,
                       cv::getStructuringElement(cv::MORPH_RECT, {7, 7}));
      basket_found = find_basket_in_mask(fallback_blue, "blue_fallback");
    }
    if (!basket_found) {
      // Last resort: the basket rim is a large bright rectangle in the
      // grayscale image. Adaptive threshold + area filtering can catch it
      // when color segmentation fails entirely.
      cv::Mat adaptive_basket;
      cv::adaptiveThreshold(gray, adaptive_basket, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C,
                            cv::THRESH_BINARY, 31, -8.0);
      cv::morphologyEx(adaptive_basket, adaptive_basket, cv::MORPH_CLOSE,
                       cv::getStructuringElement(cv::MORPH_RECT, {5, 5}));
      basket_found = find_basket_in_mask(adaptive_basket, "adaptive_basket");
    }
    debug = bgr.clone();
                                          ...
                                          cv::Mat &debug)
  {
    ...
    // Minimal 1-px erosion: removes the black stroke edge that findContours
    // includes in the hole boundary. A nut flush against the frame loses
    // only its outermost pixel; the fractional edge-clip check below
    // determines whether the result is usable.
    cv::erode(roi, roi, cv::getStructuringElement(cv::MORPH_RECT, {1, 1}));
    cv::Mat edge_distance;
    cv::distanceTransform(roi, edge_distance, cv::DIST_L2, cv::DIST_MASK_PRECISE);
    // Keep raw (un-blurred) gray inside a narrow border band so Gaussian
    // smoothing cannot merge a near-border nut with the black stroke.
    raw_gray.copyTo(gray, edge_distance <= 1.5);
    ...
    cv::bitwise_and(adaptive_mask, roi, adaptive_mask);
    cv::bitwise_and(blackhat_mask, roi, blackhat_mask);
    // A finer-scale adaptive pass helps separate adjacent nuts that the
    // large-block threshold merges into a single component.
    cv::Mat fine_adaptive_mask;
    {
      const int fine_block = std::max(3, config_.adaptive_block_size / 2);
      cv::adaptiveThreshold(gray, fine_adaptive_mask, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C,
                            cv::THRESH_BINARY_INV, fine_block | 1, config_.adaptive_c);
      cv::bitwise_and(fine_adaptive_mask, roi, fine_adaptive_mask);
      cv::morphologyEx(fine_adaptive_mask, fine_adaptive_mask, cv::MORPH_OPEN,
                       cv::getStructuringElement(cv::MORPH_ELLIPSE,
                                                  {config_.nut_mask_open_kernel_size,
                                                   config_.nut_mask_open_kernel_size}));
      cv::morphologyEx(fine_adaptive_mask, fine_adaptive_mask, cv::MORPH_CLOSE,
                       cv::getStructuringElement(cv::MORPH_ELLIPSE,
                                                  {config_.nut_mask_close_kernel_size,
                                                   config_.nut_mask_close_kernel_size}));
      cv::bitwise_and(fine_adaptive_mask, roi, fine_adaptive_mask);
    ...
    // The fine-scale adaptive pass catches nuts that the large-block threshold merges.
    for (const cv::Mat &candidate_mask : {blackhat_mask, adaptive_mask, fine_adaptive_mask}) {

      std::vector<std::vector<cv::Point>> contours;
      cv::findContours(candidate_mask, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
      // When two nuts touch, the single large contour encloses both. Split
      // oversized blobs with a distance-transform watershed so that each
      // sub-region can be classified independently.
      {
        constexpr double kOversizeAreaFactor = 2.0;
        std::vector<std::vector<cv::Point>> split_contours;
        for (const auto &c : contours) {
          const double c_area = cv::contourArea(c);
          if (c_area > config_.min_nut_area_px * kOversizeAreaFactor) {
            cv::Rect bbox = cv::boundingRect(c);
            bbox &= cv::Rect(0, 0, candidate_mask.cols, candidate_mask.rows);
            if (bbox.area() <= 0) { split_contours.push_back(c); continue; }
            cv::Mat local_mask = cv::Mat::zeros(bbox.size(), CV_8UC1);
            std::vector<std::vector<cv::Point>> shifted{{}};
            for (const auto &p : c) shifted.back().push_back(p - bbox.tl());
            cv::drawContours(local_mask, shifted, 0, 255, cv::FILLED);
            cv::Mat dist;
            cv::distanceTransform(local_mask, dist, cv::DIST_L2, cv::DIST_MASK_PRECISE);
            double dmin, dmax;
            cv::Point dmin_loc, dmax_loc;
            cv::minMaxLoc(dist, &dmin, &dmax, &dmin_loc, &dmax_loc);
            if (dmax < 2.0) { split_contours.push_back(c); continue; }
            // Find seed regions on the 8-bit threshold before converting
            // to CV_32S for watershed. findContours only supports CV_8UC1.
            cv::Mat mark8;
            cv::threshold(dist, mark8, dmax * 0.45, 255, cv::THRESH_BINARY);
            mark8.convertTo(mark8, CV_8UC1);
            std::vector<std::vector<cv::Point>> mark_contours;
            cv::findContours(mark8, mark_contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
            if (mark_contours.size() < 2) { split_contours.push_back(c); continue; }
            mark8.convertTo(mark8, CV_32S);
            for (std::size_t mi = 0; mi < mark_contours.size(); ++mi)
              cv::drawContours(mark8, mark_contours, static_cast<int>(mi), cv::Scalar(mi + 1), cv::FILLED);
            cv::Mat ws_input;
            cv::cvtColor(local_mask, ws_input, cv::COLOR_GRAY2BGR);
            cv::watershed(ws_input, mark8);
            for (int label = 1; label <= static_cast<int>(mark_contours.size()); ++label) {
              cv::Mat component = (mark8 == label);
              std::vector<std::vector<cv::Point>> sub;
              cv::findContours(component, sub, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
              for (auto &sc : sub) {
                for (auto &p : sc) p += bbox.tl();
                if (cv::contourArea(sc) >= config_.min_nut_area_px * 0.5) split_contours.push_back(sc);
              }
            }
          } else {
            split_contours.push_back(c);
          }
        }
        contours = std::move(split_contours);
      }
      ...
      for (const auto &contour : contours) {
        ...
        // A nut touching the frame has a few contour points near the ROI
        // edge; a true frame fragment has most of its points there.  Only
        // reject when >40 % of the contour lies within 2 px of the border.
        {
          std::size_t clipped_pts = 0;
          for (const auto &p : contour)
            if (edge_distance.at<float>(p) <= 2.0F) ++clipped_pts;
          if (static_cast<double>(clipped_pts) / contour.size() > 0.40) {
...
: config_(config), frame_hold_ms_(frame_hold_ms), basket_hold_ms_(frame_hold_ms * 2.0)
{
  config_.validate();
                                     ...
                                     const PointNormalizer &normalize)
{
  ...
  if (basket_hold_ms_ == 0.0 || static_cast<double>(stamp_ms - basket_stamp_ms_) > basket_hold_ms_) {
    basket_.clear();
    basket_angle_deg_ = 0;
  }
  ...
  // Hold the basket across short detection gaps. The physical basket
  // never moves, so a single missed frame should not drop slot positions.
  if (result.basket_found) {
    basket_ = result.geometry.basket;
    basket_angle_deg_ = result.geometry.basket_angle_deg;
    basket_stamp_ms_ = stamp_ms;
    result.basket_reused = false;
  } else if (!basket_.empty()) {
    result.geometry.basket = basket_;
    result.geometry.basket_angle_deg = basket_angle_deg_;
    result.basket_found = true;
    result.basket_reused = true;
  }
  return result;
}
}  // namespace lbot_vision
