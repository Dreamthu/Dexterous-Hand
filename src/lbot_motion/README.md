...
- 订阅左臂关节状态和 `left_arm/pose_states` 末端位姿；
- 封装左臂 `MoveJ`、`MoveJP`、`MoveL`、正解和逆解服务；
- 支持等待新收到的左臂关节状态，供运动前复查；
- 支持等待实际末端位姿连续到位，校验坐标系、时间戳、XYZ 距离和四元数角度误差，
  可返回最后一帧实测 XYZ/RPY 供控制层生成相对运动目标；
- 发布左 O6 手的位置、速度和力矩；
- 只提供左臂急停请求，不会自动解除急停。
...
关节与末端状态回调。到位检查的目标、误差阈值、连续帧数和超时由控制层传入，
设备层不决定后续动作。参数单位如下：
...
`joint_follow()` 封装左臂七关节弧度话题，`fresh_left_joints()` 提供按接收时间判断的新鲜反馈。
轨迹规划、按时插值发送和跟随误差监测由 `lbot_control` 完成。话题发布成功只表示 ROS 消息已发出，
不是控制器的执行确认；驱动当前忽略 FollowJoint.follow，停止仍使用急停接口。
