...
int main(int argc, char** argv) 
{
    ...
    auto node = std::make_shared<rclcpp::Node>("demo_lkls_i1_movej_node");
    ...
    auto send = [&](auto client, std::vector<float> joints, bool block)
    {
        ...
        auto status = rclcpp::spin_until_future_complete(node->get_node_base_interface(), future, std::chrono::seconds(10));
    ...
    send(left_movej_client, {-1.2, 0.5, 0.5, -1.5, -1.2, 0.0, 0.0}, true);
    std::cout << "left start move 2" << std::endl;
    send(left_movej_client, {-1.2, 0.5, 0.0, -1.5, -1.2, 0.0, 0.0}, true);

    std::cout << "right start move 1" << std::endl;
    send(right_movej_client, {1.2, -0.5, -0.5, -1.5, 1.2, 0.0, 0.0}, true);
    std::cout << "right start move 2" << std::endl;
    send(right_movej_client, {1.2, -0.5, -0.0, -1.5, 1.2, 0.0, 0.0}, true);
