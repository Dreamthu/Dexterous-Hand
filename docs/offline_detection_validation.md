...
2026-09-07 同日后续修改新增中央配置 `enable_hough_fallback`，默认值为 `false`。当前默认行为不再将
0、1、2 个轮廓观测用 Hough 补足到三颗；合成空框/一颗/两颗契约已分别改为期望
0/1/2。下文“旧版选择数量”及 Hough 误检表格保留为历史基线证据，不代表当前默认配置。
`max_frame_area_ratio` 保持 `0.30`，`max_nut_area_px` 保持 `100000.0`。
...
该段是任务一验证时的结论。随后已默认禁用 Hough，并增加固定任务专用的阶段身份/抓取状态；
最新行为与新增测试见 [nut_sequence.md](nut_sequence.md) 和 [nut_sequence_changes.md](nut_sequence_changes.md)。
