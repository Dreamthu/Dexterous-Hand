...
候选来源为 `blackhat`、`adaptive`、`hough`；Hough 候选没有真实外轮廓，contour 为空。
拒绝原因包括面积、圆度、实心度、长宽比、靠近框线、ROI 外、重复，以及
`legacy_three_target_cap`（仅前三个被接受，**本轮刻意保留旧行为**）。
`nuts` 是旧策略选择的目标，不是全部合理目标；不能拿数量直接判定任务完成。
...
- 旧节点仍缓存 RGB/depth，尚未处理重复帧、时效、不同步、TF 混帧和历史话题结果失效。
  消费者不得把旧 PoseArray 当成当前有效抓取指令。

完整文件清单、测试结果和提交步骤见 [本次修改与提交说明](offline_detection_changes.md)。

验证结果见 [2026-09-07 验证报告](offline_detection_validation.md)。离线契约测试通过不代表识别准确性通过，已确认空框 Hough 误检。
