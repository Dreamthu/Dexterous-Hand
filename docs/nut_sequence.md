...
- 第 0 阶段期望看见 3 颗，按像素半径从大到小绑定 ID 1、2、3。
- 只有收到 ID 1 的明确 `complete` 后，才进入期望 2 颗的阶段；此时剩余两颗按像素半径从大到小映射为 ID 2、3。
...
这些仍是像素规则。相机视角、距离或倾角变化后需要现场重新验证，不代表真实物理尺寸分类。
...
它只接收时间戳、图像尺寸和 `cv::Vec3f(u,v,radius)` 观测，不依赖 ROS、相机 SDK、深度、文件系统或机械臂。ROS 节点只负责消息转换、图像时效检查、深度定位和事件 service。
