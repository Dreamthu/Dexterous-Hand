...
class ExtrinsicCalibrationTests(unittest.TestCase):
    def test_right_wrist_configuration_matches_fixed_camera_setup(self) -> None:
        ...
        self.assertEqual(config["frames"]["base_frame"], "base_link")
        self.assertEqual(config["frames"]["robot_pose_child_frame"], "Arm_Tip")
        self.assertEqual(config["robot"]["model_end_link"], "R8_Link")
        self.assertEqual(config["tf_publish"]["camera_root_frame"], "camera_link")
